...
#
# Reads WAF_PORT from .env. HTTP only (TLS is Phase 4 — do not test HTTPS/8443 here).
...
STATUS_1="$(curl -s -o /dev/null -w '%{http_code}' "${WAF_URL}/api/users/check-username?username=1%27%20OR%20%271%27%3D%271")"
check_status "1. SQLi query param (942100)" "403" "${STATUS_1}"

# 2. XSS in query param -> 403 (rule 941100)
STATUS_2="$(curl -s -o /dev/null -w '%{http_code}' "${WAF_URL}/?x=<script>alert(1)</script>")"
check_status "2. XSS query param (941100)" "403" "${STATUS_2}"

# 3. Path traversal in QUERY STRING (not raw path — Pitfall 4) -> 403 (9301xx)
STATUS_3="$(curl -s -o /dev/null -w '%{http_code}' "${WAF_URL}/api/users/check-username?username=../../../../etc/passwd")"
check_status "3. Path traversal in query string (9301xx)" "403" "${STATUS_3}"

# 4. Scanner UA -> 403 (rule 913100)
STATUS_4="$(curl -s -o /dev/null -w '%{http_code}' -A "sqlmap/1.7" "${WAF_URL}/api/users/check-username?username=test")"
check_status "4. Scanner User-Agent sqlmap/1.7 (913100)" "403" "${STATUS_4}"

# 5. Legit signup POST (JSON) -> 201, must NOT be blocked
STATUS_5="$(curl -s -o /dev/null -w '%{http_code}' -X POST "${WAF_URL}/api/auth/signup" \
...
LOGIN_RESPONSE="$(curl -s -i -X POST "${WAF_URL}/api/auth/login" \
  -H "Content-Type: application/json" \
...
STATUS_7="$(curl -s -o /dev/null -w '%{http_code}' "${WAF_URL}/")"
